...
p=0.60
test_data_imbalance <- generate_clustered_data(p=p, n_point  =n.size_test )
...
for (r in 1:R) {
  ...
  weighted_knn_original <- balanced_knn(train=train_data[, -ncol(train_data)], trainlabels =  as.numeric(as.character(train_data$y)), test = test_data_balanced[,- ncol(test_data_balanced)], k=k_range, p_hat = p_hat )
  ...
  #####
  k_values <- expand.grid(k = seq(2, 500, by=5))  # Ensure the column name is 'k'
...
n.size_test <- 10000
p=0.80
test_data_imbalance <- generate_clustered_data(n_points=n.size_test, p=p)
...
ggsave("AM-Exm4-KNN.pdf", plot = p, width = 6, height = 4, dpi = 300)##AM_risk_standard_knn_with_balanced_test------------------------------
...
ggsave("AM-Exm4-logis.pdf", plot = p, width = 6, height = 4, dpi = 300)##AM_risk_standard_knn_with_balanced_test------------------------------
