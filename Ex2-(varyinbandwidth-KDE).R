...
save(AM_risk_KNN, file="AM-EX2-bandwidthvary-check.RData")
...
ggplot(AM_risk_KNN, aes(x = method, y = x, group = d, color = d)) +
  ...
  labs(x = "Bandwidth", y = "AM risk", title = "Example 2") +
  theme(
  ...
  scale_color_discrete(
    name = "Model",
    breaks = labels,
    labels <- c("SMOTE(k=5)","SMOTE(k=v)", "KDE","KDE(SB)")
  )




##Combined map for Example 2 and 3
