...
eta1 <- exp(Xmat%*%beta1)
y<- rgp(n.size_test,loc=0, scale = eta1, shape = 0.5)
...
for (r in 1:R) {
  ...
  beta1<- c(0,1, rep(0, p-1))
  eta1 <- exp(Xmat%*%beta1)
  y<- rgp(n.size,loc=0, scale = eta1, shape = 0.5)
  data <- cbind(X, y = y)
...
ggsave("AM-Exm2-KNN.pdf", plot = p, width = 6, height = 4, dpi = 300)##AM_risk_standard_rf_with_balanced_test------------------------------
...
ggsave("AM-Exm2-logis.pdf", plot = p, width = 6, height = 4, dpi = 300)##AM_risk_standard_rf_with_balanced_test------------------------------
